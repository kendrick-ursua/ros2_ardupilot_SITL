#!/usr/bin/env python3
"""
LawinTracker Hover Test Node (hover_test.py)
=============================================
...
Run:
  ros2 run lawin_mavros hover_test
"""
...
class HoverTestNode(Node):
    ...
    def __init__(self):
        ...
        # Locked hold — set ONCE after climb, never updated
        self.hold_x   = None
        self.hold_y   = None
        self.hold_z   = None
        self.hold_yaw = 0.0
        ...
        self.create_timer(0.05, self._publish_hold)   # 20Hz position
    ...
    def _main_loop(self):
        ...
        if self.stage == STAGE_LOCKED:
            ...
            if self.rc_tracking_enabled:
                # _follow_target handles pos_pub internally
                self._follow_target()
            else:
                self._send_velocity(0.0, 0.0, 0.0)
                self._lock_hold_position()
    ...
    def _lock_hold_position(self):
        """Lock X/Y/Z ONCE after climb. Never changes again."""
        if self._local_pose is not None:
            ...
            self.hold_yaw = math.atan2(siny, cosy)
            self.get_logger().info(
                f"Position LOCKED: "
                f"x={self.hold_x:.2f} y={self.hold_y:.2f} "
                f"z={self.hold_z:.2f} "
                f"yaw={math.degrees(self.hold_yaw):.1f}deg")
        else:
            self.get_logger().warn(
                "No local pose — cannot lock position!")

    def _publish_hold(self):
        """20Hz — publish locked position + current yaw.
        X/Y/Z never change. Only yaw rotates during tracking.
        """
        if self.stage not in (STAGE_POSHOLD, STAGE_FOLLOW):
            return
        if self.hold_x is None:
            return
        half = self.hold_yaw * 0.5
        msg = PoseStamped()
        msg.header.stamp    = self.get_clock().now().to_msg()
        msg.header.frame_id = 'map'
        msg.pose.position.x = self.hold_x
        msg.pose.position.y = self.hold_y
        msg.pose.position.z = self.hold_z
    ...
    def _follow_target(self):
        """Yaw-only — updates hold_yaw, _publish_hold sends it."""
        if not self.has_target or self.hold_x is None:
            return

        err_x = self.offset_x
        if abs(err_x) < STOP_ZONE:
            self.smooth_yaw = self._smooth(self.smooth_yaw, 0.0)
            self.get_logger().info(
                f"HOLD — centered! (err_x={err_x:+.0f}px)",
                throttle_duration_sec=1.0)
            return
        ...
        # Accumulate into hold_yaw — published by _publish_hold
        self.hold_yaw += self.smooth_yaw * 0.05
        self.hold_yaw = math.atan2(
            math.sin(self.hold_yaw),
            math.cos(self.hold_yaw))

        h = "RIGHT" if err_x > 0 else "LEFT"
        self.get_logger().info(
            f"YAW {h} | err_x={err_x:+.0f}px | "
            f"rate={self.smooth_yaw:+.3f} | "
            f"heading={math.degrees(self.hold_yaw):.1f}deg",
